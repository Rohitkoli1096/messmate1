...
app.get("/api/health", (req, res) =>
  res.json({
    status: "MessMate API is running 🍛",
    static_path: uploadPath,
  })
);
    ...
    for (const s of students) {
      await db.query(
        ...
        [
          s.id,
          "Don't forget dinner! 🌙",
          "Dinner is served from 7 PM. Scan your QR to mark attendance!",
          "attendance",
        ]
      );
    }
...
// 6. GET LOCAL IP (FOR MOBILE ACCESS) ✅ ADDED
function getLocalIP() {
...
// 7. START SERVER (UPDATED) ✅
const PORT = process.env.PORT || 5001;
const localIP = getLocalIP();

app.listen(PORT, "0.0.0.0", () => {
  console.log("\n🍛 MessMate API running!\n");

  console.log(`Local:            http://localhost:${PORT}`);
  console.log(`On Your Network:  http://${localIP}:${PORT}`);

  console.log(`\n🖼️ Static images served from: ${uploadPath}\n`);
});